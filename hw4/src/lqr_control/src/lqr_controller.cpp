...
*/
  ...
  std::cout<<"Matrix A value "<<endl<<matrix_a_<<endl;
  /*
...
bool LqrController::ComputeControlCommand(
  ...
  */
  ...
  steer_angle_feedback=-NormalizeAngle(steer_angle_feedback);
  ...
  cout<<"steer_angle value is"<<steer_angle<<endl;
...
void LqrController::UpdateMatrix(const VehicleState &vehicle_state) {
  matrix_a_(1, 1) = matrix_a_coeff_(1, 1)/vehicle_state.velocity;
  matrix_a_(1, 3) = matrix_a_coeff_(1, 3)/vehicle_state.velocity;
  matrix_a_(3, 1) = matrix_a_coeff_(3, 1)/vehicle_state.velocity;
  matrix_a_(3, 3) = matrix_a_coeff_(3, 3)/vehicle_state.velocity;
  Matrix matrix_i = Matrix::Identity(matrix_a_.cols(), matrix_a_.cols());
...
double LqrController::ComputeFeedForward(const VehicleState &localization,
      ...
      curvature_error = (wheelbase_ * ref_curvature + 
      kv * v * v * ref_curvature - matrix_k_(0, 2) *(lr_ * ref_curvature -lf_ * mass_ * v * v * ref_curvature / 2 / cr_ / wheelbase_));
      curvature_error=NormalizeAngle(curvature_error);
      // cout<<"curvature_error value is"<<curvature_error<<endl;
...
void LqrController::ComputeLateralErrors(const double x, const double y,
     ...
     Nearest_Point=QueryNearestPointByPosition(x,y);
     double point_y_in_vehicle = -(Nearest_Point.x-x)*sin(theta)+(Nearest_Point.y-y)*cos(theta);
     double lateral_error_check;
     if(point_y_in_vehicle>=0){
      lateral_error_check=-sqrt(PointDistanceSquare(Nearest_Point,x,y));
    }
         
    else{
      lateral_error_check=sqrt(PointDistanceSquare(Nearest_Point,x,y));
    }
     lat_con_err->lateral_error=lateral_error_check;
     ...
     lat_con_err->heading_error_rate= linear_a*sin(lat_con_err->heading_error);
     cout<<"theta value is"<<theta<<endl;
     cout<<"Nearest_Point.heading value is"<<Nearest_Point.heading<<endl;
    //  cout<<"lateral_error value is"<<lat_con_err->lateral_error<<endl;
     cout<<"heading_error value is"<<lat_con_err->heading_error<<endl;
    //  cout<<"lateral_error_rate value is"<<lat_con_err->lateral_error_rate<<endl;
    //  cout<<"heading_error_rate value is"<<lat_con_err->heading_error_rate<<endl;
    //  cout<<"lateral_error_check is"<<lateral_error_check<<endl;
