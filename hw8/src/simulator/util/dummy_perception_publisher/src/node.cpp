...
DummyPerceptionPublisherNode::DummyPerceptionPublisherNode()
  ...
  dynamic_object_pub_ =
      pnh_.advertise<autoware_perception_msgs::DynamicObjectWithFeatureArray>(
          "output/dynamic_object", 1, true);
  pointcloud_pub_ =
      pnh_.advertise<sensor_msgs::PointCloud2>("output/points_raw", 1, true);

  object_sub_ = pnh_.subscribe(
      "input/object", 1, &DummyPerceptionPublisherNode::objectCallback, this);

  timer_ = nh_.createTimer(ros::Duration(0.1),
                           &DummyPerceptionPublisherNode::timerCallback, this);
  pnh_.param<double>("visible_range", visible_range_, double(100.0));
  pnh_.param<double>("detection_successful_rate", detection_successful_rate_,
                     0.8);
...
void DummyPerceptionPublisherNode::timerCallback(const ros::TimerEvent &) {
  ...
  try {
    ...
    ros_base_link2map = tf_buffer_.lookupTransform(
        /*target*/ "base_link", /*src*/ "map", current_time,
        ros::Duration(0.5));
  ...
  for (size_t i = 0; i < objects_.size(); ++i) {
    if (detection_successful_rate_ <
        detection_successful_random(random_generator_))
      continue;
    const double std_dev_x =
        std::sqrt(objects_.at(i).initial_state.pose_covariance.covariance[0]);
    const double std_dev_y =
        std::sqrt(objects_.at(i).initial_state.pose_covariance.covariance[7]);
    const double std_dev_z =
        std::sqrt(objects_.at(i).initial_state.pose_covariance.covariance[14]);
    ...
    tf2::Transform tf_map2moved_object;
    {
      ...
      tf2::fromMsg(ros_object_origin2moved_object,
                   tf_object_origin2moved_object);
    }
    tf2::fromMsg(objects_.at(i).initial_state.pose_covariance.pose,
                 tf_map2object_origin);
    ...
    tf2::Transform tf_moved_object2noised_moved_object(
        noised_quat, tf2::Vector3(x_random(random_generator_),
                                  y_random(random_generator_), 0.0));
    tf2::Transform tf_base_link2noised_moved_object;
    tf_base_link2noised_moved_object = tf_base_link2map * tf_map2moved_object *
    ...
    feature_object.object.state.pose_covariance =
    ...
    tf2::toMsg(tf_base_link2noised_moved_object,
    ...
    double dist = std::sqrt(tf_base_link2moved_object.getOrigin().x() *
                                tf_base_link2moved_object.getOrigin().x() +
                            tf_base_link2moved_object.getOrigin().y() *
                                tf_base_link2moved_object.getOrigin().y());
  ...
  pcl::PointCloud<pcl::PointXYZ>::Ptr merged_pointcloud_ptr(
      new pcl::PointCloud<pcl::PointXYZ>);
    ...
    for (size_t i = 0; i < v_pointcloud.size(); ++i) {
      ...
      for (size_t j = 0; j < v_pointcloud.at(i)->size(); ++j) {
        Eigen::Vector3i grid_cordinates = ray_tracing_filter.getGridCoordinates(
            v_pointcloud.at(i)->at(j).x, v_pointcloud.at(i)->at(j).y,
            v_pointcloud.at(i)->at(j).z);
        int grid_state;
        if (ray_tracing_filter.occlusionEstimation(grid_state,
                                                   grid_cordinates) != 0)
        ...
        if (grid_state == 1) {  // occluded
          ...
          ray_traced_merged_pointcloud_ptr->push_back(
              v_pointcloud.at(i)->at(j));
        }
      }
      pcl::toROSMsg(
          *ray_traced_pointcloud_ptr,
          output_dynamic_object_msg.feature_objects.at(i).feature.cluster);
      output_dynamic_object_msg.feature_objects.at(i)
          .feature.cluster.header.frame_id = "base_link";
      output_dynamic_object_msg.feature_objects.at(i)
          .feature.cluster.header.stamp = current_time;
    }
...
void DummyPerceptionPublisherNode::createObjectPointcloud(
  ...
  const double r = std::sqrt(tf_base_link2moved_object.getOrigin().x() *
                                 tf_base_link2moved_object.getOrigin().x() +
                             tf_base_link2moved_object.getOrigin().y() *
                                 tf_base_link2moved_object.getOrigin().y());
  auto getBaseLinkTo2DPoint = [tf_base_link2moved_object](
                                  double x, double y) -> pcl::PointXYZ {
  ...
  pcl::PointCloud<pcl::PointXYZ> horizontal_pointcloud;
  {
    ...
    for (double x = -1.0 * (length / 2.0); x <= ((length / 2.0) + epsilon);
         x += step) {
  ...
  }
  {
    ...
    for (double y = -1.0 * (width / 2.0); y <= ((width / 2.0) + epsilon);
         y += step) {
      horizontal_candidate_pointcloud.push_back(getBaseLinkTo2DPoint(x, y));
    }
  }
  ...
  // 2D ray tracing
  size_t ranges_size = std::ceil((horizontal_max_theta - horizontal_min_theta) /
                                 horizontal_theta_step);
  std::vector<double> horizontal_ray_traced_2d_pointcloud;
  horizontal_ray_traced_2d_pointcloud.assign(
      ranges_size, std::numeric_limits<double>::infinity());
  const int no_data = -1;
  ...
  for (size_t i = 0; i < horizontal_candidate_pointcloud.points.size(); ++i) {
    double angle = std::atan2(horizontal_candidate_pointcloud.at(i).y,
                              horizontal_candidate_pointcloud.at(i).x);
    double range = std::hypot(horizontal_candidate_pointcloud.at(i).y,
                              horizontal_candidate_pointcloud.at(i).x);
  ...
  for (const auto &pointcloud_index :
       horizontal_ray_traced_pointcloud_indices) {
    if (pointcloud_index != no_data) {
      // generate vertical point
      horizontal_pointcloud.push_back(
          horizontal_candidate_pointcloud.at(pointcloud_index));
      const double distance =
          std::hypot(horizontal_candidate_pointcloud.at(pointcloud_index).x,
                     horizontal_candidate_pointcloud.at(pointcloud_index).y);
      for (double vertical_theta = vertical_min_theta;
           vertical_theta <= vertical_max_theta + epsilon;
           vertical_theta += vertical_theta_step) {
        const double z = distance * std::tan(vertical_theta);
        if (min_z <= z && z <= max_z + epsilon) {
          pcl::PointXYZ point;
          point.x = horizontal_candidate_pointcloud.at(pointcloud_index).x +
                    x_random(random_generator_);
          point.y = horizontal_candidate_pointcloud.at(pointcloud_index).y +
                    y_random(random_generator_);
      ...
      for (size_t i = 0; i < objects_.size(); ++i) {
        if (objects_.at(i).id.uuid == msg->id.uuid) {
          ...
          try {
            geometry_msgs::TransformStamped ros_input2map;
            ros_input2map = tf_buffer_.lookupTransform(
                /*target*/ msg->header.frame_id, /*src*/ "map",
          ...
          tf2::fromMsg(msg->initial_state.pose_covariance.pose,
                       tf_input2object_origin);
          tf_map2object_origin =
              tf_input2map.inverse() * tf_input2object_origin;
          dummy_perception_publisher::Object object;
          objects_.at(i) = *msg;
          tf2::toMsg(tf_map2object_origin,
                     objects_.at(i).initial_state.pose_covariance.pose);
          break;
        }
      }
      break;
