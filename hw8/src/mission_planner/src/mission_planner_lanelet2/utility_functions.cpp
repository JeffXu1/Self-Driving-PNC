...
bool getClosestLanelet(const geometry_msgs::Pose& search_pose,
  ...
  lanelet::BasicPoint2d search_point(search_pose.position.x,
                                     search_pose.position.y);
  std::vector<std::pair<double, lanelet::Lanelet>> nearest_lanelet =
      lanelet::geometry::findNearest(lanelet_map_ptr_->laneletLayer,
                                     search_point, 1);
  if (nearest_lanelet.empty()) {
    ROS_ERROR_STREAM("Failed to find the closest lane!"
                     << std::endl
                     << "search point: " << toString(search_pose) << std::endl);
    return false;
  }
  if (nearest_lanelet.front().first > distance_thresh) {
    ROS_ERROR_STREAM("Closest lane is too far away!"
                     << std::endl
                     << "search point: " << toString(search_pose) << std::endl
