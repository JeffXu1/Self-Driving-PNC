...
void RouteHandler::setRouteLanelets(
  ...
  if (!path_lanelets.empty()) {
    ...
    start_lanelets_ = lanelet::utils::query::getAllNeighbors(routing_graph_ptr_,
                                                             first_lanelet);
    auto last_lanelet = path_lanelets.back();
    goal_lanelets_ = lanelet::utils::query::getAllNeighbors(routing_graph_ptr_,
                                                            last_lanelet);
  }
  ...
  for (const auto& lane : path_lanelets) {
    ...
    for (const auto& left_relation : left_relations) {
      ...
      } else if (left_relation.relationType ==
  ...
  for (const auto& candidate_id : candidate_lanes_id) {
    lanelet::ConstLanelet lanelet =
        lanelet_map_ptr_->laneletLayer.get(candidate_id);
    ...
    while (!previous_lanelets.empty() && is_connected_to_candidate_prev &&
      ...
      for (const auto& prev_lanelet : previous_lanelets) {
        if (route_lanelets_id.find(prev_lanelet.id()) !=
            route_lanelets_id.end()) {
        ...
        if (candidate_lanes_id.find(prev_lanelet.id()) !=
            candidate_lanes_id.end()) {
    ...
    while (!following_lanelets.empty() && is_connected_to_candidate_next &&
      ...
      for (const auto& next_lanelet : following_lanelets) {
        if (route_lanelets_id.find(next_lanelet.id()) !=
            route_lanelets_id.end()) {
        ...
        if (candidate_lanes_id.find(next_lanelet.id()) !=
            candidate_lanes_id.end()) {
...
lanelet::ConstLanelets RouteHandler::getLaneletSequenceUpTo(
  ...
  std::reverse(lanelet_sequence_backward.begin(),
...
lanelet::ConstLanelets RouteHandler::getLaneletSequence(
  ...
  // loop check
  if (lanelet_sequence_forward.empty() > 1 &&
      lanelet_sequence_backward.empty() > 1) {
    if (lanelet_sequence_backward.back().id() ==
        lanelet_sequence_forward.front().id()) {
      return lanelet_sequence_forward;
    }
  }

  lanelet_sequence.insert(lanelet_sequence.end(),
...
lanelet::ConstLanelets RouteHandler::getPreviousLaneletSequence(
  ...
  auto right_relations = lanelet::utils::query::getAllNeighborsRight(
      routing_graph_ptr_, first_lane);
  ...
  auto left_relations = lanelet::utils::query::getAllNeighborsLeft(
      routing_graph_ptr_, first_lane);
...
lanelet::ConstLanelets RouteHandler::getLaneSequence(
    const lanelet::ConstLanelet& lanelet) const {
  ...
  lane_sequence.insert(lane_sequence.end(), lane_sequence_up_to.begin(),
                       lane_sequence_up_to.end());
  lane_sequence.insert(lane_sequence.end(), lane_sequence_after.begin(),
                       lane_sequence_after.end());
...
lanelet::ConstLanelets RouteHandler::getLaneSequenceUpTo(
  ...
  while (ros::ok()) {
    ...
    lanelet::ConstLanelets current_lanelet_section =
        getNeighborsWithinRoute(current_lanelet);
    lanelet::ConstLanelets prev_lanelet_section =
        getNeighborsWithinRoute(prev_lanelet);
...
lanelet::ConstLanelets RouteHandler::getLaneSequenceAfter(
  ...
  while (ros::ok()) {
    ...
    lanelet::ConstLanelets current_lanelet_section =
        getNeighborsWithinRoute(current_lanelet);
    lanelet::ConstLanelets next_lanelet_section =
        getNeighborsWithinRoute(next_lanelet);
...
bool RouteHandler::getNextLaneletWithinRoute(
  ...
  lanelet::ConstLanelets following_lanelets =
      routing_graph_ptr_->following(lanelet);
...
bool RouteHandler::getPreviousLaneletWithinRoute(
  ...
  lanelet::ConstLanelets previous_lanelets =
      routing_graph_ptr_->previous(lanelet);
